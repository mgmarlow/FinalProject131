...
# Abstract
The purpose of this project is to classify risky and safe cars based off of a number of attributes acquired from the "Automobile" data set. The methods that I use are primarily tree-based; I use four different classification trees and judge by the misclassification error which tree method is the best one to use. I begin with a standard binary recursive partitioning method and examine the tree based off of the most basic merits. From there, I partition the data into a training set and test set, and create a new tree based off of the training set. Using the `predict()` function from the `tree` library, I predict a different tree using the test set. I then proceed to use K-fold cross validation to make a newly pruned tree, and examine the error of this tree compared to the original, where I defined my own training and test sets. The next two methods that I use are bootstrapping methods, mainly random forests and boosting, which I compare to the pruned tree achieved during cross-validation. 
...
My dataset is found on UC Irvine's Machine Learning Repository, and is accredited to Jeffery C. Schlimmer. The data was donated in 1987. The sources for this data are outlined within my R code, as well as on the UCI page for the dataset.
...
Since there is a significantly significant relationship between `price` and `symboling` (in terms of the slope of the regression line), there is a strong possibility that `price` is a strong contributing factor to `symboling`. I will test this hypothesis later in the project to see if my prediction holds up to the actual classification.
